  ...
  try {
    ...
    try {
      if (action === "list") {
        ...
        console.log(
          `Workflows listados: ${totalReceived} recebidos, ${totalExcluded} excluídos (arquivados).`,
        );
        ...
        let tagIds = (Array.isArray(workflowData.tags) ? workflowData.tags : [])
          .map((item: any) => {
            ...
            const name = String(item?.name ?? catalogTag?.name ?? "")
              .trim()
              .toLowerCase();
        ...
        if (wantedTag) {
          ...
          if (!found) {
            ...
            if (!createRes.ok) {
              return new Response(
                JSON.stringify({ error: `n8n tag create failed: ${createRes.status}` }),
                {
                  status: 502,
                  headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (!response.ok) {
          const errorText = await response.text();
          return new Response(
            JSON.stringify({ error: `n8n set tags failed: ${response.status} ${errorText}` }),
            {
              status: 502,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
            },
        ...
        const parentFolderKeys = firstWorkflow?.parentFolder
          ? Object.keys(firstWorkflow.parentFolder)
          : [];
        ...
        return new Response(
          JSON.stringify({
            ...
            hasParentFolderId:
              properties.includes("parentFolderId") ||
              detailedProperties.includes("parentFolderId"),
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
        );
      } else if (action === "crm-sync" || action === "crm-send-to-group") {
        if (!M7_WEBHOOK_TOKEN) {
        ...
        if (action === "crm-send-to-group" && (!leadId || typeof leadId !== "string")) {
        ...
        const endpoint =
          action === "crm-sync"
            ? "f5c9e51a-422b-436a-8a6e-2cf73a3ffd78/m7-crm/sync"
            : "f0abde0b-e54b-46ba-9440-cd825045769a/m7-crm/whatsapp";
        const response = await fetch(
          `https://projetopessoal-n8n.h574he.easypanel.host/webhook/${endpoint}`,
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify(action === "crm-sync" ? {} : { leadId }),
            signal: controller.signal,
          },
        );
        ...
        if (!response.ok) {
          return new Response(
            JSON.stringify({ error: "n8n error", data, status: response.status }),
            {
        ...
        const response = await fetch(
          `https://projetopessoal-n8n.h574he.easypanel.host/webhook/m7-financeiro-lancamentos/m7-financeiro/lancamentos`,
          {
            method: "POST",
            headers: {
              "x-m7-token": M7_WEBHOOK_TOKEN,
        ...
        const response = await fetch(
          `https://projetopessoal-n8n.h574he.easypanel.host/webhook/m7-financeiro-receber/m7-financeiro/receber`,
          {
            method: "POST",
            headers: {
              "x-m7-token": M7_WEBHOOK_TOKEN,
        ...
        const response = await fetch(
          `https://projetopessoal-n8n.h574he.easypanel.host/webhook/f50fb807-63ad-48d8-b24d-1ada58c9d5a4/m7-financeiro/editar`,
          {
            method: "POST",
            headers: {
              "x-m7-token": M7_WEBHOOK_TOKEN,
